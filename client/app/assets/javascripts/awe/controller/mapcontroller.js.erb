...
//            obj.alpha = 0.3;
    ...
    var setArmyPosition = function(view, pos, army) {
      view.setCenter(AWE.Geometry.createPoint(pos.x, pos.y-view.frame().size.height/2));
...
//                  view.displayObject().alpha=0.3;
    ...
    that.runloop = function() {
      ...
      if(AWE.Map.Manager.isInitialized()) {
        ...
        if (_windowChanged || _needsDisplay || _loopCounter % 12 == 0 || that.modelChanged() || _actionViewChanged || animating) {
          ...
          for (var i=0; i < _stages.length; i++) {
            if (stageUpdateNeeded[i] || _windowChanged || i==1) {
